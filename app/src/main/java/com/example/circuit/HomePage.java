...
public class HomePage extends AppCompatActivity implements ItemClickListener {
    ...
    public void addItemToList(){
        ...
        //when the button has been clicked to save the details
        saveDetails.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                //getting the values of the new items created
                ...
                //this checks if there is an image in the imageview before trying to upload
                 if(hasImage(takenPicture)) {
                    if(name.isEmpty() || description.isEmpty() || price.isEmpty()){
                        Toast.makeText(HomePage.this, "fields cannot be empty", Toast.LENGTH_LONG).show();
                    }
                    else{
                        //this converts the image to a version that would allow you to upload it to firebase
    ...
    //this function uploads the picture to firebase cloud storage and sets a reference to the realtime database
    private void uploadPicture(byte[] arr, String name) {

        //this line gets the link to the storage reference and creates a file with the value of name, which is the item name
        StorageReference imgRef2 = storage.getReference("Images/" + name);
        //String choice;
